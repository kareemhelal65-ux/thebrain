...
async function getEnabledCategories(companyId) {
  ...
  if (dbError) throw new Error(`Failed to fetch enabled categories: ${dbError.message}`);
  ...
  try {
    ...
    if (!compError && company && company.integrations) {
      const integrations = company.integrations;
      const providerToCategories = {
        ...
        dropbox: ['storage'],
        onedrive: ['storage'],
        'google workspace': ['storage', 'communications']
      };

      for (const [provider, status] of Object.entries(integrations)) {
        if (status && status.connected) {
          const cats = providerToCategories[provider.toLowerCase()];
          if (cats) {
    ...
    console.warn('[companyConfig] Failed to resolve integrations from company table:', err.message);
  }

  // Always enable storage category so that native drafting/document creation works
